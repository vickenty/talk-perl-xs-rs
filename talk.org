...
* What is Rust
  #+BEGIN_NOTES
  Rust is a new programming language, that targets roughly the same niche as C++, being a low-level language with a lot of manual control, while at the same time providing higher-level tools to the programmer. One of main goals of Rust eliminate bugs caused by improper use of memory pointers, the bane of all reasonably complicated C or C++ programs.
  #+END_NOTES
* C example
